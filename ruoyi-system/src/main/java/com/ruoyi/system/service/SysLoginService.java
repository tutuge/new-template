        ...
        if (user != null && user.getGoogleKey() != null && !"".equals(user.getGoogleKey())) {
            ...
            if (googleCode != null && !"".equals(googleCode)) {
                if (isNumeric(googleCode)) {
                    boolean flag = GoogleAuthUtil.checkCode(user.getGoogleKey(), Integer.parseInt(googleCode));
                    if (!flag) {
                        recordLogininfor(username, Constants.LOGIN_FAIL, "Google验证码错误");
                ...
                recordLogininfor(username, Constants.LOGIN_FAIL, "Google验证码不能为空");
