<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="128px">
      <el-form-item label="群名" prop="groupTitle">
        <el-input
...
<!--            icon="Plus"-->
<!--            @click="handleAdd"-->
<!--            v-hasPermi="['bot:groupRecord:add']"-->
<!--        >新增-->
<!--        </el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
...
<!--            :disabled="single"-->
<!--            @click="handleUpdate"-->
<!--            v-hasPermi="['bot:groupRecord:edit']"-->
<!--        >修改-->
<!--        </el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
...
<!--            :disabled="multiple"-->
<!--            @click="handleDelete"-->
<!--            v-hasPermi="['bot:groupRecord:remove']"-->
<!--        >删除-->
<!--        </el-button>-->
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['bot:groupRecord:export']"
        >导出
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="groupRecordList" @selection-change="handleSelectionChange">
      <!--      <el-table-column type="selection" width="55" align="center"/>-->
      <el-table-column label="消息的id" align="center" prop="id" v-if="true"/>
      <el-table-column label="群名" align="center" prop="groupTitle"/>
      ...
      <el-table-column label="消息时间" align="center" prop="messageDate"/>
      <el-table-column label="消息内容" align="center" prop="messageText"/>
      <el-table-column label="回复" align="center" prop="aiReply">
        <template #default="scope">
          {{ scope.row.aiReply ? scope.row.aiReply.length > 20 ? scope.row.aiReply.substring(0,20) + "....." : scope.row.aiReply : ""}}
        </template>
      </el-table-column>
      <el-table-column label="回复状态" align="center" prop="replayStatus" :formatter="(row)=>replyStatusFormatter(row.replayStatus)"/>
<!--      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">-->
<!--        <template #default="scope">-->
<!--          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"-->
<!--                     v-hasPermi="['bot:groupRecord:edit']">修改-->
<!--          </el-button>-->
<!--          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"-->
<!--                     v-hasPermi="['bot:groupRecord:remove']">删除-->
<!--          </el-button>-->
<!--        </template>-->
<!--      </el-table-column>-->
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="groupRecordRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="群名" prop="groupTitle">
          <el-input v-model="form.groupTitle" placeholder="请输入群名"/>
        </el-form-item>
        <el-form-item label="用户昵称" prop="userName">
        ...
        <el-form-item label="回复" prop="aiReply">
...
import {
  ...
  listGroupRecord,
  updateGroupRecord
} from "@/api/bot/groupRecord";
import {getCurrentInstance, reactive, ref} from "vue";
import {replyStatusFormatter} from "@/utils/format"
...
function getList() {
  loading.value = true;
  listGroupRecord(queryParams.value).then(response => {
    groupRecordList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("groupRecordRef");
}
...
function handleUpdate(row) {
  ...
  getGroupRecord(_id).then(response => {
    loading.value = false;
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["groupRecordRef"].validate(valid => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.id != null) {
        updateGroupRecord(form.value).then(response => {
        ...
        addGroupRecord(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除群提问编号为"' + _ids + '"的数据项？').then(function () {
    loading.value = true;
    return delGroupRecord(_ids);
  }).then(() => {
    loading.value = true;
    getList();
...
function handleExport() {
  proxy.download('bot/groupRecord/export', {
    ...queryParams.value
